...
class GameSurface(ScreenHandle):
    ...
    def draw_object(self, sprite, coord):
        ...
        self.blit(
            sprite,
            (
                (coord[0] - min_x) * self.game_engine.sprite_size,
...
class ProgressBar(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        self.blit(
            font.render(f"Hero at {self.engine.hero.position}", True, colors["black"]),
            (250, 0),
        )

        self.blit(
            font.render(f"{self.engine.level} floor", True, colors["black"]), (10, 0)
        )
        ...
        self.blit(
            font.render(f"{self.engine.hero.level}", True, colors["black"]), (360, 30)
        )
        self.blit(
            font.render(f"{self.engine.hero.gold}", True, colors["black"]), (360, 70)
        )
        ...
        self.blit(
            font.render(f'{self.engine.hero.stats["strength"]}', True, colors["black"]),
            (480, 30),
        )
        self.blit(
            font.render(f'{self.engine.hero.stats["luck"]}', True, colors["black"]),
            (480, 70),
        )

        self.blit(font.render(f"SCORE", True, colors["black"]), (550, 30))
        self.blit(
            font.render(f"{self.engine.score:.4f}", True, colors["black"]), (550, 70)
        )
        if self.successor is not None:
            canvas.blit(self.successor, self.next_coord)
            self.successor.draw(canvas)
...
class HelpWindow(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        if self.engine.show_help:
            ...
            for i, text in enumerate(self.data):
                self.blit(
                    font1.render(text[0], True, ((128, 128, 255))), (50, 50 + 30 * i)
                )
                self.blit(
                    font2.render(text[1], True, ((128, 128, 255))), (150, 50 + 30 * i)
                )
