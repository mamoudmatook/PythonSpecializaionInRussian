...
def convert(amount, cur_from, cur_to, date, requests):
    ...
    if cur_from != "RUR":
        ru_sum = amount * Decimal(
            str(
                soup.find("CharCode", text=cur_from).find_next_sibling("Value").string
            ).replace(",", ".")
        )
        ru_sum = ru_sum / Decimal(
            str(
    ...
    if cur_to != "RUR":
        out_sum = ru_sum / Decimal(
            str(
                soup.find("CharCode", text=cur_to).find_next_sibling("Value").string
            ).replace(",", ".")
        )
        out_sum = out_sum * Decimal(
            str(
